...
import { getFavoriteProductIDs, removeFavoriteProductID, saveFavoriteProductID } from 'services/LocalStorageService';
...
export default class FavoritesStore implements Favorites {
    ...
    async getFavoritesByLocalStorage(userFavorites: Favorite[] = []) {
        ...
        let uniqueFavoriteProductIDs = favoriteProductIDs.filter(id => !userFavorites.some(item => item.product.id === id));
        if (!uniqueFavoriteProductIDs.length) {
            return [];
        }
        const {
            data: { data: products }
        } = await fetchProducts({ filters: { id: uniqueFavoriteProductIDs } });
    ...
    clearFavorites() {
        let favoriteProductIDs = getFavoriteProductIDs();
        this.items = this.items.filter(item => favoriteProductIDs.some(productID => productID === item.product.id));
    }
}

...
export default class CartStore implements Cart {
	...
	async getShoppingCartByLocalStorage(
		userCartItems: ShoppingCartItem[] = []
	) {
		let cartProductIDs = getCartProductIDs();
		let uniqueCartProductIDs = cartProductIDs.filter(
			(id) => !userCartItems.some((item) => item.product.id === id)
		);
		...
		const {
			...
			filters: { id: uniqueCartProductIDs },
			populate: ['images'],
		});
	...
	clearShoppingCart() {
		...
		this.items = this.items.filter((item) =>
			cartProductIDs.some((productID) => productID === item.product.id)
		);
	}
}

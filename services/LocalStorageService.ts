...
export const saveFavoriteProductID = (id: number) => {
    ...
    localStorage.setItem('favoriteProductIDs', JSON.stringify([...productIDs, id]))
}

export const removeFavoriteProductID = (id: number) => {
    let productIDs = getFavoriteProductIDs();
    localStorage.setItem('favoriteProductIDs', JSON.stringify(productIDs.filter((productID) => productID !== id)))
}
...
export const removeCartProductID = (id: number) => {
    let productIDs = getCartProductIDs();
    localStorage.setItem('cartProductIDs', JSON.stringify(productIDs.filter((productID) => productID !== id)))
}

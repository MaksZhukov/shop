...
const ProductPage = ({ data, page, relatedProducts }: Props) => {
	...
	return (
		<>
			<HeadSEO
				title={data.seo?.title || page.title}
				description={data.seo?.description || page.description}
				keywords={data.seo?.keywords || keywordsContent}
			></HeadSEO>
			<Container>
				<WhiteBox padding='2em'>
					...
					<Typography padding='1em 0' variant='body1'>
						...
						городах Беларуси - Гродно, Минск, Брест, Гомель, Могилев, Витебск. Так же мы сообщаем что
						работаем во всех городах и деревнях, просто доставка займет немного больше времени. Будьте
						уверены, мы приложим все силы, что бы ваш товар - {data.h1 || data.name} был доставлен
						максимально быстро.
					</Typography>
				</WhiteBox>

...
const Home: NextPage<Props> = ({
    ...
    const getSparePartsHref = () => {
        const asPath = router.asPath;
        let path = '/spare-parts';
        if (brand && !model) {
            path += `/${brand}${asPath.replace(`brand=${brand}`, '')}`;
        } else if (model) {
            path += `/${brand}/model-${model}${asPath.replace(`brand=${brand}`, '').replace(`&model=${model}`, '')}`;
        } else {
            path = asPath;
        }
        console.log(path);
    ...
    const filtersBtn = (
        <NextLink href={getSparePartsHref()}>
                            ...
                            <WhiteBox marginX="0.5em" key={item.id}>
                                <NextLink href={`/spare-parts/${item.slug}`}>
                                    <Image
                                        ...
                                        style={isTablet ? { margin: 'auto' } : {}}></Image>
                                </NextLink>
                            </WhiteBox>
                        ))}
                </Slider>
